...
&НаКлиенте
Асинх Процедура ЭкспортироватьВремяВБитрикс(Команда)
...
Процедура ПолучитьВремяClockifyНаСервере()
	...
	БитриксЗатраченноВремя = ОбработкаОбъект.Bitrix_Задача_ПолучитьЗатраченноеВремя(Bitrix24WebHook, СокрЛП(БитриксНомерЗадачи), Истина);
...
Функция ВремяДляБитрикс()
	...
	БитриксПользовательID = Формат(ПараметрыСеанса.Автор.ПользовательБитрикс.Код, "ЧГ=");
	...
	Для Каждого СтрЗатраченноеВремя ИЗ ЗатраченноеВремя Цикл
		...
		ДеталиОЗатраченномВремени.Вставить("USER_ID",		БитриксПользовательID);	
