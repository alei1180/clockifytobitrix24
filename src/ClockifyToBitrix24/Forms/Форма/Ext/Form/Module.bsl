...
Процедура ПолучитьВремяClockifyНаСервере()
	...
	БитриксЗатраченноВремя = ОбработкаОбъект.Bitrix_Задача_ПолучитьЗатраченноеВремя(Bitrix24WebHook,
																					СокрЛП(БитриксНомерЗадачи),
																					BitrixUserID,
																					Истина);
	Если НЕ ЗначениеЗаполнено(БитриксЗатраченноВремя) Тогда
		Возврат;
	КонецЕсли;
