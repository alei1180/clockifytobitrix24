...
//    * ПотраченоСекунд - Число - Количество затраченных секунд
...
Функция ПрочитатьПараметрыЗаписиClockify(ClockifyДанные)
	...
	ПараметрыЗаписиОВремени.Вставить("ПотраченоСекунд");
	...
	КонецЕсли;
	ПараметрыЗаписиОВремени.ПотраченоСекунд = ПараметрыЗаписиОВремени.ДатаОкончания - ПараметрыЗаписиОВремени.ДатаНачала;
	ПараметрыЗаписиОВремени.ЗатраченоеВремя = Дата(1,1,1) + ПараметрыЗаписиОВремени.ПотраченоСекунд;
...
Процедура ЗаполнитьТЗЗатраченноеВремя(ClockifyДанныеПоЗатраченномуВремени, БитриксЗатраченноВремя, МассивНомеровЗадач)
	...
	Для Каждого ClockifyДанные Из ClockifyДанныеПоЗатраченномуВремени Цикл
		...
		ОтборВремяБитрикс.Вставить("Секунды", ДанныеОВремениКлокифай.ПотраченоСекунд);
...
Процедура ОшибкиПересчитатьПодвал()
	...
	Для Каждого СтрОшибка Из Ошибки Цикл
		...
		ПотраченоСекунд = СтрОшибка.ДатаОкончания - СтрОшибка.ДатаНачала;
		ИтогоЗатраченоСекунд = ИтогоЗатраченоСекунд + ПотраченоСекунд;
