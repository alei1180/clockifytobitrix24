...
Процедура ПолучитьВремяClockify(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ClockifyAPIkey) Тогда
		СообщитьПользователю("Не удалось получить время: не заполнено поле Clockify API Key", Неопределено, "ClockifyAPIkey");
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Bitrix24WebHook) Тогда
		СообщитьПользователю("Не удалось получить время: не заполнено поле Bitrix24 Webhook", Неопределено, "Bitrix24WebHook");
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(БитриксНомерЗадачи) Тогда
		СообщитьПользователю("Не удалось получить время: заполните поле Номер БЗ", Неопределено, "БитриксНомерЗадачи");
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(BitrixUserID) Тогда
		СообщитьПользователю("Не удалось получить время: заполните поле Bitrix24 User", Неопределено, "BitrixUserID");
		Возврат;
	КонецЕсли;
