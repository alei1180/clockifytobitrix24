...
Процедура ПриОткрытии(Отказ) 
	...
	Элементы.ЗатраченноеВремяЗатраченоеВремя.Ширина = 13; 
...
Процедура ВыбратьПользователяБитрикс(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Bitrix24WebHook) Тогда
		Сообщить("Для выбора пользователя необходимо сначала заполнить Bitrix WebHook");
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьВремяClockify(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ClockifyAPIkey) Тогда
		Сообщить("Не удалось получить время: не заполнено поле Clockify API Key");
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Bitrix24WebHook) Тогда
		Сообщить("Не удалось получить время: не заполнено поле Bitrix24 Webhook");
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(БитриксНомерЗадачи) Тогда
		Сообщить("Не удалось получить время: заполните поле Номер БЗ");
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(BitrixUserID) Тогда
		Сообщить("Не заполнено поле Bitrix24 User");
		Возврат;
	КонецЕсли;
	
	ЗатраченноеВремя.Очистить();
	ПолучитьВремяClockifyНаСервере();
...
Процедура ЭкспортироватьВремяВБитрикс(Команда)
	...
	Иначе
		Если ЗатраченноеВремя.Количество() > 0 Тогда
			Сообщить(СтрШаблон("По задаче №%1 все записи о времени уже выгружены в битрикс 24",СокрЛП(БитриксНомерЗадачи)));
		Иначе
			Сообщить("Нечего выгружать");
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьТЗЗатраченноеВремя(ClockifyДанныеПоЗатраченномуВремени, БитриксЗатраченноВремя) 
	
	ТЗНазваниеЗадачБитрикс = НазваниеЗадачБитрикс(ClockifyДанныеПоЗатраченномуВремени);
	Если НЕ ЗначениеЗаполнено(ТЗНазваниеЗадачБитрикс) Тогда
		Сообщить("В комментариях по затраченному времени Clockify, не указано ни одного номера задачи");
		Возврат;
	КонецЕсли;
		
	БЗНомер = СокрЛП(БитриксНомерЗадачи);
	...
	Часы = Цел(ИтогоЗатраченоСекунд/3600);
	Минуты = Цел((ИтогоЗатраченоСекунд-Часы*3600)/60);
	Секунды = ИтогоЗатраченоСекунд-Часы*3600-Минуты*60;
	
	Элементы.ЗатраченноеВремяЗатраченоеВремя.ТекстПодвала = СтрШаблон("%1 ч. %2 м. %3 с.", Часы, Минуты, Секунды); 
...
&НаСервере
Процедура ПолучитьВремяClockifyНаСервере()
	...
	ClockifyДанныеПользователя = ОбработкаОбъект.Clockify_ДанныеПользователя(АдресСервера, ClockifyAPIkey, Метод);
	Если ClockifyДанныеПользователя.Свойство("activeWorkspace") И ClockifyДанныеПользователя.Свойство("id") Тогда
		ClockifyactiveWorkspace = ClockifyДанныеПользователя["activeWorkspace"];
		ClockifyUserID			= ClockifyДанныеПользователя["id"];
	Иначе
		Возврат;
	КонецЕсли;
	
	Метод = СтрШаблон("/api/v1/workspaces/%1/user/%2/time-entries/", ClockifyactiveWorkspace, ClockifyUserID);
	ClockifyДанныеПоЗатраченномуВремени = ОбработкаОбъект.Clockify_ДанныеПоЗатраченномуВремени(АдресСервера, ClockifyAPIkey, Метод, 1000);
	
	БитриксЗатраченноВремя = ОбработкаОбъект.Bitrix_Задача_ПолучитьЗатраченноеВремя(Bitrix24WebHook, СокрЛП(БитриксНомерЗадачи), BitrixUserID, Истина);
	Если БитриксЗатраченноВремя = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ClockifyДанныеПоЗатраченномуВремени) Тогда
		ЗаполнитьТЗЗатраченноеВремя(ClockifyДанныеПоЗатраченномуВремени, БитриксЗатраченноВремя);
	Иначе
		Сообщить("Время clockify не удалось получить.");
	КонецЕсли;
...
Функция ВремяДляБитрикс()
	...
	Для Каждого СтрЗатраченноеВремя ИЗ ЗатраченноеВремя Цикл
		...
		МассивЗаписи.Добавить(СокрЛП(БитриксНомерЗадачи));
...
Процедура ЭкспортироватьВремяВБитриксНаСервере()
	...
	Для Каждого ЗаписьЗатраченногоВремени Из МассивВремениДляБитрикс Цикл 
		...
		АйдиДобавленнойЗаписиБитрикс = ОбработкаОбъект.Bitrix_Задача_ДобавитьЗатраченноеВремя(Bitrix24WebHook, НомерЗадачи, ДеталиЗаписиОЗатраченномВремени);	
	...
	Сообщить(СтрШаблон("В задачу битрикс №%1 добавлено %2 записей, ошибок при загрузке: %3.", СокрЛП(БитриксНомерЗадачи),ЗагруженоВБитрикс, НеЗагруженоВБитрикс));
...
Функция ЭтоНомерЗадачи(Описание)
	...
	Для Сч = 1 по СтрДлина(Описание) Цикл
		...
		Если НЕ Найти(Цифры, ТекСимв) > 0 Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
//  МассивНомеровЗадач  - Массив - массив строк с номерами задач найденных в комментарии Clockify
//
// Возвращаемое значение:
//   Булево   - Истина если запись не прошла проверку
//
&НаСервере
Функция ПропуститьЗапись(ОписаниеЗатраченногоВремени, Знач ДатаНачала, Знач ДатаОкончания)
	...
	ЭтоЗаписьПоДругойЗадаче = СтрНайти(ОписаниеЗатраченногоВремени, СокрЛП(БитриксНомерЗадачи)) = 0;
...
Функция НомераЗадачБитрикс(ClockifyДанныеПоЗатраченномуВремени)
	...
	Для каждого ClockifyДанные из ClockifyДанныеПоЗатраченномуВремени Цикл
		...
		НомерЗадачиИОписаниеНЕРазделеныПробелом = СтрНайти(ClockifyДанные.description, " ", НаправлениеПоиска.СНачала) = 0;	
		Если НомерЗадачиИОписаниеНЕРазделеныПробелом Тогда
			Продолжить;
		КонецЕсли;
...
&НаСервере
Функция НазваниеЗадачБитрикс(ClockifyДанныеПоЗатраченномуВремени)
	...
	Если ЗначениеЗаполнено(МассивНомеровЗадач) Тогда
		ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
		ТЗНазваниеЗадачБитрикс = ОбработкаОбъект.Bitrix_Задача_ПолучитьИнформацию(Bitrix24WebHook, МассивНомеровЗадач,Истина);
	КонецЕсли;
...
&НаСервере
Функция ПользователяБитрикс(ИДПользователяБитрикс)
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	
	Возврат ОбработкаОбъект.Bitrix_Пользователь(Bitrix24WebHook, ИДПользователяБитрикс);
...
Процедура ПослеВводаIDПользователяБитрикс(ИДПользователяБитрикс, Параметры) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ПользовательБитрикс) Тогда
		Сообщить(СтрШаблон("Не удалось получить пользователя по ID %1", формат(ИДПользователяБитрикс, "ЧГ=")));
		Возврат;
	КонецЕсли;
	
	Если НЕ ПользовательБитрикс.ACTIVE Тогда
		Сообщить(СтрШаблон("Пользователь с ID %1 не является активным", формат(ИДПользователяБитрикс, "ЧГ=")));
		Возврат;
	КонецЕсли;
